...
class add_ejemplos(Action):
    def name(self) -> Text:
        return "action_mirar_ejemplos"
    ...
    async def run(
        ...
        intent = tracker.latest_message['intent'].get('name')       # obtenemos el intent
        ejemplo = tracker.latest_message.get('text')                # y el nuevo ejemplo

        limiteMIN = 0.5             # porcentaje minimo que debe de parecerse el ejemplo a los existentes
        limiteMAX = 0.7             # porcentaje maximo
        maxEjem = 10                # numero maximo de ejemplos que puede contener un intent  
        ...
        with open (".\\data\\nlu.yml", "r+", encoding="utf-8") as nlu:            
            ...
            if parecido == True: # si es parecido no se incluye
                print("No se va a añadir '{}', poco o muito parecido".format(ejemplo))
